...
import { Eye, Layers, Sparkles, Check, ArrowUpRight } from "lucide-react";
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-14 text-left">
          {filteredSamples.map((item, idx) => (
            <div
              key={idx}
              className="group bg-slate-950/70 rounded-3xl border border-slate-800/80 p-6 shadow-xl hover:border-cyan-500/40 hover:-translate-y-1.5 transition-all duration-300 flex flex-col justify-between backdrop-blur-md"
            >
              <div>
              ...
              <div className="pt-4 border-t border-slate-800/80 flex items-center justify-between text-xs font-medium text-slate-400">
                <span className="flex items-center gap-1.5 text-emerald-400 font-semibold">
                  <Check className="w-4 h-4" /> Arquivo PDF HD
                </span>
